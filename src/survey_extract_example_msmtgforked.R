...
#   distinct() %>%
#   mutate(analysis = "kp") %>%
#   filter(variable %in% c( "age", "sex", "gender", "gender2")) %>% ### removing residence is to get round the character/integer binding row problem later. A bridge to cross when we start looking at spatial issues
#   rename(var_raw = var_label_raw) %>%
#   filter(file_type %in% c("MSM", "TGW", "TGM", "TG"))
...
# Extracting TGW
# paths <- list.files("~/Imperial College London/HIV Inference Group - WP - Documents/Data/KP/Individual level data/", recursive = TRUE, pattern = ".rds", full.names = TRUE) %>%
#   lapply(., grep, pattern= "CFSW|PWUD|PLACE|NPP|MSW|code|Anne|v1|v2|cpt_fin|dbn_fin|jhb_fin|old|.rdsobj|TIPVAL|hsh_rdsat_format.csv", value = TRUE, invert = TRUE) %>%
#   unlist()
...
# FOR THE MSM TG COMPARED AGE DIST
# msm_recoded <- all_recoded[names(all_recoded) %in% c("COD2022BBS_MSM", "COD2022BBS_TG", "KEN2016ACA_MSM", "KEN2016ACA2_MSM", "LSO2019BBS_MSM", "LSO2019BBS_TGW", "MLI2014BBS_MSM", "MOZ2021BBS_MSM", "MWI2016ACA_MSM", "MWI2019BBS_MSM", "MWI2020BBS_TG", "NAM2019BBS_MSM", "RWA2015BBS_MSM", "RWA2018BBS_MSM", "SLE2021BBS_MSM", "SLE2021BBS_TGW", "SWZ2020BBS_TGW", "SWZ2020BBS_MSM", "UGA2012BBS_MSM", "ZAF2013BBS_MSM", "ZAF2016ACA_MSM", "ZAF2019BBS_MSM", "ZAF2019BBS_TGW")]
# 
# 
# msm_recoded <- lapply(msm_recoded, function(df) {
#   df %>%
#     # mutate(across(starts_with("coupon"), ~as.integer(extract_numeric(.)))) %>%
#     mutate(age = if ("age" %in% names(.))
#       as.integer(extract_numeric(age)))
# })
# 
# msm_df <- msm_recoded %>% 
#   map_df(~select(.x, any_of(c("survey_id", "age", "sex", "gender"))))
# 
# write_csv(msm_df, "~/Imperial College London/HIV Inference Group - WP - Documents/Data/KP/Individual level data/00Admin/Data extracts/msmtg_age_df_1211.csv")
...
rds2 <- rds_adjust(all_recoded2$SWZ2020BBS_FSW, "SWZ2020BBS_FSW", variable_recode)

# Testing out survey city grouping structure
# swzcities <- list(
# manzini <- all_recoded2[["SWZ2020BBS_FSW"]] %>% filter(survey_city == "Manzini"),
# mbabane <- all_recoded2[["SWZ2020BBS_FSW"]] %>% filter(survey_city == "Mbabane"),
# pigpeak <- all_recoded2[["SWZ2020BBS_FSW"]] %>% filter(survey_city == "Pig's Peak"),
# ngalo <- all_recoded2[["SWZ2020BBS_FSW"]] %>% filter(survey_city == "Nhlagano"),
# siteki <- all_recoded2[["SWZ2020BBS_FSW"]] %>% filter(survey_city == "Siteki")
# )
# 
# names(swzcities) <- rep("SWZ2020BBS_FSW", 5)
# 
# swzrds <- swzcities %>% 
#   Map(rds_adjust,
#       df = .,
...
#   arrange(category) %>% 
#   group_by(category) %>% 
#   mutate(sum_estimate = sum(estimate))

...
child_age_death_vars <- c(paste0("b7_0", 1:9), paste0("b7_1", 1:9), "b7_20")
...
mics_paths <- file.path(
  "sites", Sys.getenv("SHAREPOINT_SITE"),
  Sys.getenv("MICS_ORDERLY_PATH"), mics_sharepoint_df$name
)
mics_files <- lapply(
  mics_paths, spud::sharepoint_download,
...
df %>%
  mutate(
    ...
    across(contains("bh"), ~ val_recode(
      .x, cur_column(), survey_id_c, dataset_type, analysis
    )),
